...
const { saveImageToDrive, removeFileFromDrive, updateImageInDrive } = require("../../integrations/google-drive");
...
router.patch("/update/:id", async (req, res) => {
  ...
  try {
    const { id } = req.params;
    const { body } = req;
    const { newImageVisor, newImageSlider } = body;
    ...
    const { imageVisor, imageSlider } = existingMedia;
    if(newImageVisor) await updateImageInDrive(imageVisor, newImageVisor);
    if(newImageSlider) await updateImageInDrive(imageSlider, newImageSlider);

    await Media.update(body, { where: { id: id } });
...
router.delete("/delete/:id", async (req, res) => {
  ...
  try {
    ...
    const { imageVisor, imageSlider } = existingMedia;
    if(imageVisor) await removeFileFromDrive(imageVisor);
    if(imageSlider) await removeFileFromDrive(imageSlider);
